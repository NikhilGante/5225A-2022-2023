...
std::string file_name= "/usd/data.txt";
std::string file_meta= "/usd/meta_data.txt";
...
Data::Data(std::string obj_name, std::string id_code, log_types log_type_param, log_locations log_location_param, bool newline, term_colours print_colour){
  this->id = id_code;
  this->name = obj_name;
...
Data task_log("tasks.txt","$01", general);
// Data controller_queue("controller.txt","$02", general,log_locations::none);
Data tracking_data("tracking.txt","$03",general);
...
Data log_d("log.txt","$08",general,log_locations::sd, false);
  ...
  if(!file.is_open()){
    ...
    for(int i = 0; i< Data::obj_list.size(); i++){
      if((Data::obj_list[i]->log_location == log_locations::sd || Data::obj_list[i]->log_location == log_locations::both) && int(Data::obj_list[i]->log_type) !=0){
        strcat(meta_data,Data::obj_list[i]->name.c_str());
        strcat(meta_data,",");
        strcat(meta_data,Data::obj_list[i]->id.c_str());
        strcat(meta_data,",");
      }
    }
...
void queue_handle(){
  ...
  // char* temp_back;
  while(true){
    if(inRange(Data::queue.size(), 0U, print_point) || logging_tmr.get_time() > print_max_time){
